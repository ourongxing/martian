...
function parseParagraph(element: md.Paragraph, options: BlocksOptions): notion.Block[] {
...
function parseBlockquote(element: md.Blockquote, options: BlocksOptions): notion.Block {
...
function parseList(element: md.List, options: BlocksOptions): notion.Block[] {
  return element.children.flatMap((item) => {
    ...
    const parsedChildren: notion.BlockWithoutChildren[] = item.children.flatMap(
      child => parseNode(child, options) as unknown as notion.BlockWithoutChildren,
    )
...
function parseTable(node: md.Table): notion.Block[] {
  ...
  const tableWidth = node.children?.length ? node.children[0].children.length : 0
...
function parseNode(node: md.FlowContent, options: BlocksOptions): notion.Block[] {
  switch (node.type) {
    case "heading":
...
export function parseBlocks(root: md.Root, options?: BlocksOptions): notion.Block[] {
  ...
  if (parsed.length > LIMITS.PAYLOAD_BLOCKS) {
    limitCallback(
      new Error(`Resulting blocks array exceeds Notion limit (${LIMITS.PAYLOAD_BLOCKS})`),
    )
  }
...
export interface RichTextOptions extends CommonOptions {
   ...
   * - `origin`: keep the origin state
   */
  nonInline?: "ignore" | "throw" | "origin"
}

export function parseRichText(root: md.Root, options?: RichTextOptions): notion.RichText[] {
  ...
  if (richTexts.length > LIMITS.RICH_TEXT_ARRAYS) {
    limitCallback(
      new Error(`Resulting richTexts array exceeds Notion limit (${LIMITS.RICH_TEXT_ARRAYS})`),
    )
  }

  return (truncate ? richTexts.slice(0, LIMITS.RICH_TEXT_ARRAYS) : richTexts).map((rt) => {
    if (rt.type !== "text") return rt

    if (rt.text.content.length > LIMITS.RICH_TEXT.TEXT_CONTENT) {
      limitCallback(
        new Error(`Resulting text content exceeds Notion limit (${LIMITS.RICH_TEXT.TEXT_CONTENT})`),
      )
      if (truncate)
        rt.text.content = `${rt.text.content.slice(0, LIMITS.RICH_TEXT.TEXT_CONTENT - 3)}...`
    }

    if (rt.text.link?.url && rt.text.link.url.length > LIMITS.RICH_TEXT.LINK_URL) {
      // There's no point in truncating URLs
      limitCallback(
        new Error(`Resulting text URL exceeds Notion limit (${LIMITS.RICH_TEXT.LINK_URL})`),
      )
    }
